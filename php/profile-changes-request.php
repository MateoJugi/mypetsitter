<?php
	...
	if ( $profileType == "sitterProfile" ) {
		...
		$profileChangesSitterID = mysqli_real_escape_string( $con, $_GET["profileChangesProfileID"] );
		...
		$profileChangesFullName = mysqli_real_escape_string( $con, $_GET["profileChangesFullName"] );
		$profileChangesEmail = mysqli_real_escape_string( $con, $_GET["profileChangesEmail"] );
		$profileChangesContactNumber = mysqli_real_escape_string( $con, $_GET["profileChangesContactNumber"] );
		$profileChangesPassword = mysqli_real_escape_string( $con, $_GET["profileChangesPassword"] );
		$profileChangesProfileID = mysqli_real_escape_string( $con, $_GET["profileChangesProfileID"] );

		if ( !empty( $profileChangesFullName ) && !empty( $profileChangesEmail ) && !empty( $profileChangesContactNumber ) && !empty( $profileChangesPassword ) ) {
			$resultUser = mysqli_query( $con, "UPDATE users SET userFullName = '" .$profileChangesFullName. "', userEmail = '" .$profileChangesEmail. "', userContactNumber = '" .$profileChangesContactNumber. "', userPassword = '" .password_hash( $profileChangesPassword, PASSWORD_DEFAULT ). "' WHERE userID = '" .$profileChangesProfileID. "'" );
		}
	}
